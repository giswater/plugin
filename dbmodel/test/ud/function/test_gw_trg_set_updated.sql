...
SELECT has_trigger('gully', 'gw_trg_set_updated', 'Table gully should have gw_trg_set_updated');
...
UPDATE gully SET updated_at = '2000-01-01', updated_by = 'nobody' WHERE gully_id = (SELECT min(gully_id) FROM gully);
SELECT ok((SELECT updated_at FROM gully WHERE gully_id = (SELECT min(gully_id) FROM gully)) > now() - interval '1 minute',
	'UPDATE gully stamps updated_at');
SELECT is((SELECT updated_by::text FROM gully WHERE gully_id = (SELECT min(gully_id) FROM gully)), current_user::text,
	'UPDATE gully stamps updated_by');
