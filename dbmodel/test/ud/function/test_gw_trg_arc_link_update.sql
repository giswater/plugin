...
INSERT INTO link (link_id, code, feature_id, feature_type, exit_id, exit_type, state, state_type, expl_id, sector_id,
	the_geom, linkcat_id, link_type, userdefined_geom)
SELECT -901, '-901', t.feature_id, 'CONNEC', p.arc_id, 'ARC', 2,
	...
	l.linkcat_id, l.link_type, false
FROM _t_op t
...
SELECT ok(
	(SELECT ST_DWithin(ST_EndPoint(l.the_geom), a.the_geom, 0.01)
	 FROM link l JOIN arc a ON a.arc_id = l.exit_id
	 WHERE l.link_id = -901),
	'state=2 connec: link endpoint follows the planned arc via exit_id'
);

SELECT ok(
	(SELECT NOT ST_Equals(ST_EndPoint(l.the_geom), ST_EndPoint(p.the_geom))
	 FROM link l, _t_plan_link p WHERE l.link_id = p.link_id),
	'state=2 connec: link endpoint moved with the planned arc'
);

CREATE TEMP TABLE _t_gully_op AS
SELECT l.link_id, l.feature_id, l.exit_id AS arc_id, l.the_geom AS link_geom
FROM link l
JOIN arc a ON a.arc_id = l.exit_id
JOIN gully g ON g.gully_id = l.feature_id
WHERE l.exit_type = 'ARC' AND l.feature_type = 'GULLY' AND l.state = 1 AND a.state = 1
  AND ST_Length(a.the_geom) > 8
  AND ST_LineLocatePoint(a.the_geom, ST_EndPoint(l.the_geom)) BETWEEN 0.25 AND 0.75
ORDER BY l.link_id
LIMIT 1;

SELECT is((SELECT count(*)::integer FROM _t_gully_op), 1, 'fixture: operative gully-link-arc found');

CREATE TEMP TABLE _t_gully_plan_arc AS
SELECT a.arc_id, a.the_geom
FROM arc a, _t_gully_op t
WHERE a.state = 1 AND a.arc_id IS DISTINCT FROM t.arc_id AND ST_Length(a.the_geom) > 8
ORDER BY ST_Distance(a.the_geom, t.link_geom)
LIMIT 1;
...
INSERT INTO link (link_id, code, feature_id, feature_type, exit_id, exit_type, state, state_type, expl_id, sector_id,
	the_geom, linkcat_id, link_type, userdefined_geom)
SELECT -902, '-902', t.feature_id, 'GULLY', p.arc_id, 'ARC', 2,
	(SELECT id FROM value_state_type WHERE state = 2 LIMIT 1),
	l.expl_id, l.sector_id,
	ST_MakeLine(ST_StartPoint(t.link_geom), ST_LineInterpolatePoint(p.the_geom, 0.5)),
	l.linkcat_id, l.link_type, false
FROM _t_gully_op t
JOIN link l ON l.link_id = t.link_id
JOIN _t_gully_plan_arc p ON true;

CREATE TEMP TABLE _t_gully_plan_link AS
SELECT link_id, the_geom FROM link WHERE link_id = -902;
...
WHERE arc_id = (SELECT arc_id FROM _t_gully_op);

SELECT ok(
	(SELECT ST_DWithin(ST_EndPoint(l.the_geom), a.the_geom, 0.01)
	 FROM link l JOIN arc a ON a.arc_id = l.exit_id
	 WHERE l.link_id = (SELECT link_id FROM _t_gully_op)),
	'state=1 gully: link endpoint stays on the moved arc'
);

SELECT ok(
	(SELECT NOT ST_Equals(ST_EndPoint(l.the_geom), ST_EndPoint(t.link_geom))
	 FROM link l, _t_gully_op t WHERE l.link_id = t.link_id),
	'state=1 gully: link endpoint moved with the arc'
);
...
WHERE arc_id = (SELECT arc_id FROM _t_gully_plan_arc);

SELECT ok(
	(SELECT ST_DWithin(ST_EndPoint(l.the_geom), a.the_geom, 0.01)
	 FROM link l JOIN arc a ON a.arc_id = l.exit_id
	 WHERE l.link_id = -902),
	'state=2 gully: link endpoint follows the planned arc via exit_id'
);

SELECT ok(
	(SELECT NOT ST_Equals(ST_EndPoint(l.the_geom), ST_EndPoint(p.the_geom))
	 FROM link l, _t_gully_plan_link p WHERE l.link_id = p.link_id),
	'state=2 gully: link endpoint moved with the planned arc'
);
