...
- when commitChanges is true, writes om_scada_graph_json via gw_fct_scada_graph_export
  ...
  group_id - generated by pgr_connectedcomponents, the minimum node_id of the ROOT nodes of each connected component
  level_id - generated by pgr_drivingDistance, represents the de depth from the ROOT nodes of each connected component
  position_id: initially generated by pgr_depthFirstSearch, then refined through Sugiyama-style barycenter iterations for hierarchical graphs.
  It represents the ordering of nodes within each level (level_id), calculated to minimise the number of crossing links in the synoptic.

	v_iterations: number of Sugiyama-style barycenter iterations to run.
	Recommended range: 4 to 24.
...
BEGIN
	...
	FROM node n1
	LEFT JOIN cat_node cn1 ON n1.nodecat_id = cn1.id
	...
	UPDATE temp_om_scada_vertice v
	SET node_type = cn.node_type
	FROM node n
	LEFT JOIN cat_node cn ON cn.id = n.nodecat_id
	...
	WITH
		...
		vertices AS (
			...
			UNION
			SELECT node_1, node_2, node_1 AS node_id, level_1 AS level_id FROM edges_to_split
			UNION
			SELECT node_1, node_2, node_2 AS node_id, level_2 AS level_id FROM edges_to_split
		),
		new_edges AS (
	...
	INSERT INTO temp_om_scada_vertice (node_id, node_type, group_id, level_id, is_real)
	SELECT g.node_1, g.node_type_1, g.group_id, g.level_id, FALSE AS is_real
	FROM temp_om_scada_graph g
	WHERE g.node_1 < 0;
