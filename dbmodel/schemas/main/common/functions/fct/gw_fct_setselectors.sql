...
BEGIN
	...
	IF v_expl_x_user is false THEN
		...
		UNION
		SELECT * FROM (SELECT DISTINCT muni_id, sector_id, unnest(expl_visibility) AS expl_id FROM node WHERE EXISTS (SELECT 1 FROM selector_state WHERE node.state = selector_state.state))
