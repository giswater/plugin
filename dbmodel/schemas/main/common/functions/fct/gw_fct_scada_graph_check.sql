...
- when commitChanges is true, writes om_scada_graph_json via gw_fct_scada_graph_export
  ...
  position_id - generated initialy by pgr_depthFirstSearch, and optimized by Sugiyama-style barycenter iterations for hierarchical graphs, represents the order of the nodes on each level (level_id), it is calculated for minimising the number of crossing lines in the synoptic
