...
- when commitChanges is true, writes om_scada_graph_json via gw_fct_scada_graph_export
  ...
  position_id - generated by pgr_depthFirstSearch, represents the order of the nodes on each level (level_id), it is calculated for minimising the number of crossing lines in the synoptic
  order_id in the table "om_scada_graph" - is the level_id of node_1
...
BEGIN
	...
	-- update group_id, level_id, position_id and order_id
	...
	UPDATE temp_om_scada_graph g
	SET order_id = n.level_id
	FROM temp_om_scada_vertice n
	WHERE g.the_geom IS NOT NULL
	AND g.node_1 = n.node_id; -- assures to update all the edges, because drivingdistance returns nodes, not edges

	-- for position_id will be taken into account the order of x and y coordinates of the v_pgr_root_vids nodes, so that the position_id is assigned from left to right
	...
	-- save an intermediate value for position_aux
	UPDATE temp_om_scada_vertice n
	...
	-- save position_id as the row_number() of position_aux, partitioned by group_id and level_id, ordered by position_aux
	UPDATE temp_om_scada_vertice n
	SET position_id = g.position_id
	FROM (
		SELECT node_id,
			row_number() OVER (PARTITION BY group_id, level_id ORDER BY position_aux) AS position_id
