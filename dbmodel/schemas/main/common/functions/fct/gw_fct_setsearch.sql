...
BEGIN
	...
	if v_section ilike 'basic_search_v2_tab_network%' then
		...
		EXECUTE 'SELECT row_to_json(row) FROM (SELECT ST_x(COALESCE(a.the_geom, ST_centroid(ST_envelope(s.the_geom)))) AS xcoord, ST_y(COALESCE(a.the_geom, ST_centroid(ST_envelope(s.the_geom)))) AS ycoord, St_AsText(COALESCE(a.the_geom, ST_centroid(ST_envelope(s.the_geom)))) FROM ve_streetaxis s join v_municipality m using(muni_id) left join ve_address a on s.id = a.streetaxis_id
