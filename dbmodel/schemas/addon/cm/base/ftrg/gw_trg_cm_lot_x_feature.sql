...
BEGIN
                ...
                ELSIF TG_OP = 'UPDATE' AND NEW IS DISTINCT FROM OLD THEN
					IF NEW.action IS DISTINCT FROM 3 AND NEW.action IS DISTINCT FROM 4 THEN
						IF v_isqfield IS TRUE THEN
							 NEW.action := 1;
					    ELSE
							IF NEW.integrated_id IS DISTINCT FROM OLD.integrated_id THEN
								NEW.action := NULL;
							ELSE 
								NEW.action := 2;
							END IF;
    ...
    IF TG_WHEN = 'AFTER' THEN
		...
		IF (SELECT COUNT(*) FROM (SELECT 1 FROM om_campaign_lot_x_node WHERE lot_id = NEW.lot_id AND status = 3
		    UNION ALL
		    SELECT 1 FROM om_campaign_lot_x_arc WHERE lot_id = NEW.lot_id AND status = 3
		    UNION ALL
		    SELECT 1 FROM om_campaign_lot_x_connec WHERE lot_id = NEW.lot_id AND status = 3
			) t) = 1 THEN
